...
class GamesController {
    ...
    public getOne(req: Request, res: Response) {
        ...
        db.any('SELECT * FROM games where id=$1', [req.params.id])
            .then((data) => {
                if (data.length > 0) {
                    console.log('la data es:**********', data);
                    res.status(200).json({
    ...
    public async create(req: Request, res: Response): Promise<void> {
        ...
        await db.none('INSERT INTO games (title, description, image) VALUES($1, $2, $3)', [req.body.title, req.body.description, req.body.image])
            .then((data) => {
                res.json(
                    {
                        message: 'Game Saved',
    ...
    public update(req: Request, res: Response) {
        ...
        db.any('UPDATE games SET title=${title}, description=${description}, image=${image} WHERE id = ${id}',
            {
                id: req.params.id,
